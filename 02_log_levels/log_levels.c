...
static int arr_count = ARR_SIZE;

//declare module parameters
module_param(my_int, int, 0444); // takes paramater name, type, and permissions
...
static int __init my_init(void){
    // printk(KERN_INFO "%s: This is info\n", module_name);
    ...
    // printk(KERN_DEBUG "%s: This is a debug message\n", module_name);
...
static void __exit my_exit(void){
    // pr_info("%s: This is info on exit\n", module_name);
    // pr_alert("%s: This is an alert on exit\n", module_name);
    // pr_warn("%s: This is a warning on exit\n", module_name);	
    // pr_notice("%s: This is a notice on exit\n", module_name);
    // pr_err("%s: This is an error on exit\n", module_name);
    // pr_emerg("%s: This is an emergency on exit\n", module_name);
    // pr_debug("%s: This is a debug message on exit\n", module_name);
}
